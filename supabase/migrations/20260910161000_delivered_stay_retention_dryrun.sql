...
with cfg as (select evidence_retention_days,history_retention_days from public.data_retention_config where singleton_id=1)
select s.id,s.vehicle_id,s.normalized_plate,s.stay_code,s.delivered_at,
case when p_phase='evidence' then s.delivered_at+make_interval(days=>cfg.evidence_retention_days) else s.delivered_at+make_interval(days=>cfg.history_retention_days) end,
...
and ((p_phase='evidence' and s.status='delivered' and s.evidence_purged_at is null and s.delivered_at+make_interval(days=>cfg.evidence_retention_days)<=now()) or (p_phase='history' and s.status in ('delivered','historical') and s.evidence_purged_at is not null and s.delivered_at+make_interval(days=>cfg.history_retention_days)<=now()))
...
update public.vehicle_stays set retention_claim_token=p_claim_token,retention_claimed_at=now(),retention_claim_phase=p_phase,updated_at=now() where id=p_stay_id and (retention_claim_token is null or retention_claimed_at<now()-interval '30 minutes');
if not found then return jsonb_build_object('claimed',false,'reason','already_claimed'); end if;
return jsonb_build_object('claimed',true,'storage_objects',c.storage_objects);end;$$;
...
returns boolean language sql security definer set search_path='' as $$update public.vehicle_stays set retention_claim_token=null,retention_claimed_at=null,retention_claim_phase=null,updated_at=now() where id=p_stay_id and retention_claim_token=p_claim_token returning true;$$;
...
delete from public.vehicle_photos where stay_id=s.id;delete from public.vehicle_evidence where stay_id=s.id;
...
return jsonb_build_object('purged',true);end;$$;
...
if not exists(select 1 from public.vehicle_stays where vehicle_id=s.vehicle_id) then delete from public.vehicles where id=s.vehicle_id and status='retrieved';v_deleted:=found;end if;
return jsonb_build_object('purged',true,'vehicle_deleted',v_deleted);end;$$;
...
returns trigger language plpgsql security definer set search_path='' as $$begin if new.status='open' and exists(select 1 from public.vehicle_stays s where s.id=new.stay_id and s.retention_claim_token is not null and s.retention_claimed_at>=now()-interval '30 minutes') then raise exception 'retention_cleanup_in_progress';end if;return new;end;$$;
...
insert into public.maintenance_tasks(task_key,function_slug,enabled,priority,timeout_ms,config) values
...
('purge_delivered_stay_history','delivered-stay-history-cleanup',false,300,60000,'{"phase":"history","limit":25,"execute":false}'::jsonb)
