-- Include project-scoped resource analytics in the daily Telegram health report.
-- Minimal presentation/data-source change only: cron schedule, recipients, thresholds,
-- health status logic and operational behavior remain unchanged.
-- API Requests keep the Supabase usage-counter semantics. Only Edge is labelled
-- as a 24-hour metric. The Analytics capture timestamp is shown explicitly.
...
declare
  ...
  v_edge_function_invocations bigint;
  v_analytics_captured_at timestamptz;
  v_status text := 'ok';
  v_status_icon text := '✅';
  v_body text;
  v_metrics jsonb;
...
begin
  ...
  select s.api_requests, s.edge_function_invocations, s.captured_at
  into v_api_requests, v_edge_function_invocations, v_analytics_captured_at
  from public.resource_usage_snapshots s
  ...
  v_metrics := jsonb_build_object(
    ...
    'edge_function_invocations_24h', v_edge_function_invocations,
    'resource_analytics_captured_at', v_analytics_captured_at
  );
  ...
  v_body :=
    ...
    || '🌐 API Requests: ' || coalesce(v_api_requests::text, '—') || E'\n'
    || '⚡ Edge Function Invocations (últimas 24 h): ' || coalesce(v_edge_function_invocations::text, '—') || E'\n'
    || case
         when v_analytics_captured_at is not null then
           '🕒 Captura Analytics: ' || to_char(v_analytics_captured_at at time zone v_cfg.timezone, 'DD/MM/YYYY HH24:MI') || E'\n'
         else
           '🕒 Captura Analytics: —' || E'\n'
       end
