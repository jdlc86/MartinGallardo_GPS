...
insert into public.database_health_config(id)
values (true)
on conflict (id) do nothing;

alter table public.database_health_config enable row level security;
revoke all on public.database_health_config from public, anon, authenticated;
...
revoke all on public.database_health_reports from public, anon, authenticated;

create or replace function public.emit_daily_database_health_report(p_force boolean default false)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
from cron.job
where jobname = 'database-health-midnight-madrid';
