...
create unique index if not exists vehicle_stays_code_key on public.vehicle_stays(stay_code);
...
create index if not exists vehicle_stays_vehicle_started_idx on public.vehicle_stays(vehicle_id, started_at desc);
create index if not exists vehicle_stays_plate_started_idx on public.vehicle_stays(normalized_plate, started_at desc);
create index if not exists vehicle_stays_retention_idx on public.vehicle_stays(status, delivered_at) where delivered_at is not null;
...
insert into public.vehicle_stays(vehicle_id, normalized_plate, stay_code, sequence_on_day, started_at, delivered_at, status, history_purge_eligible_at)
select
  v.id,
  coalesce(v.normalized_plate, regexp_replace(upper(v.plate), '[^A-Z0-9]', '', 'g')),
  coalesce(v.normalized_plate, regexp_replace(upper(v.plate), '[^A-Z0-9]', '', 'g')) || '-' || to_char((v.created_at at time zone 'Europe/Madrid')::date, 'YYYYMMDD') || '-001',
  1,
  v.created_at,
  v.retrieved_at,
  case when v.status = 'retrieved' then 'delivered' else 'active' end,
  case when v.retrieved_at is not null then v.retrieved_at + make_interval(days => (select history_retention_days from public.data_retention_config where singleton_id = 1)) else null end
from public.vehicles v
where not exists (select 1 from public.vehicle_stays s where s.vehicle_id = v.id)
on conflict (stay_code) do nothing;

-- Disputes belong to a stay, not to the physical plate forever. Keep vehicle_id for compatibility/reporting.
alter table public.vehicle_disputes add column if not exists stay_id uuid null references public.vehicle_stays(id) on delete cascade;
...
alter table public.vehicle_disputes alter column stay_id set not null;
drop index if exists public.vehicle_disputes_one_open_per_vehicle;
create unique index if not exists vehicle_disputes_one_open_per_stay on public.vehicle_disputes(stay_id) where status = 'open';
create index if not exists vehicle_disputes_stay_history_idx on public.vehicle_disputes(stay_id, opened_at desc);

create or replace function public.create_vehicle_stay(p_vehicle_id uuid, p_normalized_plate text, p_started_at timestamptz default now())
...
begin
  if length(v_plate) < 4 then raise exception 'invalid_plate'; end if;
  ...
  select coalesce(max(sequence_on_day), 0) + 1 into v_seq
  from public.vehicle_stays
  where normalized_plate = v_plate and (started_at at time zone 'Europe/Madrid')::date = v_day;

  insert into public.vehicle_stays(vehicle_id, normalized_plate, stay_code, sequence_on_day, started_at, status)
  values (p_vehicle_id, v_plate, v_plate || '-' || to_char(v_day, 'YYYYMMDD') || '-' || lpad(v_seq::text, 3, '0'), v_seq, p_started_at, 'active')
...
create or replace function public.mark_vehicle_stay_delivered(p_stay_id uuid, p_delivered_at timestamptz default now())
returns public.vehicle_stays
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_row public.vehicle_stays;
begin
  select history_retention_days into v_history_days from public.data_retention_config where singleton_id = 1;
  ...
  set delivered_at = p_delivered_at,
      ...
      updated_at = now()
  where id = p_stay_id and status <> 'purged'
  returning * into v_row;
...
set search_path = ''
as $$
  select exists (select 1 from public.vehicle_disputes d where d.stay_id = p_stay_id and d.status = 'open');
$$;
