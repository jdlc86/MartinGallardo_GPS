-- A Factory Reset clears operational booking state, but the singleton writer
-- state is structural. Recreate it for the preserved Owner so a clean install
-- remains immediately usable and parking_booking_dashboard never returns a
-- null permission after reset.
...
begin
  ...
  -- Structural booking permission: the preserved Owner is always the initial
  -- read/write holder after a reset. Admins may transfer it later normally.
  insert into public.parking_booking_write_state(
    id, holder_telegram_user_id, epoch,
    granted_by_telegram_user_id, granted_at, updated_at
  )
  values (1, p_owner_id, 1, p_owner_id, now(), now())
  on conflict (id) do update set
    holder_telegram_user_id=excluded.holder_telegram_user_id,
