-- Source-control the existing daily database-health report.
-- Live behavior is intentionally unchanged: pg_cron runs hourly in UTC and
-- the function emits exactly once at 00:00 Europe/Madrid, covering DST.

create table if not exists public.database_health_config (
  id boolean primary key default true check (id = true),
  timezone text not null default 'Europe/Madrid',
  database_quota_bytes bigint not null default 524288000 check (database_quota_bytes > 0),
  warning_percent numeric not null default 80 check (warning_percent > 0 and warning_percent < 100),
  critical_percent numeric not null default 90 check (critical_percent > 0 and critical_percent <= 100),
  quota_source text not null default 'Supabase Free plan · 500 MB database size',
  updated_at timestamptz not null default now()
);

create table if not exists public.database_health_reports (
...
alter table public.database_health_config enable row level security;
alter table public.database_health_reports enable row level security;

revoke all on table public.database_health_config from public, anon, authenticated;
revoke all on table public.database_health_reports from public, anon, authenticated;
grant all on table public.database_health_config to service_role;
grant all on table public.database_health_reports to service_role;

insert into public.database_health_config(
  id, timezone, database_quota_bytes, warning_percent, critical_percent, quota_source
)
values (
  true,
  'Europe/Madrid',
  ...
  80,
  90,
  'Supabase Free plan · 500 MB database size'
)
...
begin
  if not exists (
    select 1 from cron.job where jobname = 'database-health-midnight-madrid'
  ) then
    perform cron.schedule(
      'database-health-midnight-madrid',
      '0 * * * *',
      'select public.emit_daily_database_health_report(false);'
    );
  end if;
end
$$;
