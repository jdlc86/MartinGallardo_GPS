-- Make reservation dashboard counters explicit: future pickups and future returns.
...
begin
  ...
  select jsonb_build_object(
    ...
    'stats',jsonb_build_object(
      'pickups_today',(select count(*) from public.parking_events pe where pe.operation='pickup' and (pe.created_at at time zone 'Europe/Madrid')::date=(now() at time zone 'Europe/Madrid')::date),
      'delivered_today',(select count(*) from public.parking_events pe where pe.operation='retrieve' and (pe.created_at at time zone 'Europe/Madrid')::date=(now() at time zone 'Europe/Madrid')::date),
