...
async function authenticate(initData:string){
  ...
  if(!Number.isFinite(authDate)||Math.abs(Date.now()/1000-authDate)>ACCESS_MAX_AGE_SECONDS)throw new Error("expired_init_data");
...
async function upsertSession(uid:number,authDate:number){
  ...
  const expiresAt=new Date((authDate+ACCESS_MAX_AGE_SECONDS)*1000).toISOString();
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const session=await upsertSession(telegramUserId,authDate);
