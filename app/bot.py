...
from app.users import list_pending_users, set_user_status
...
async def user_access_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        target = await set_user_status(
            actor=actor, target_telegram_user_id=target_id, status=status
        )
    ...
    await query.edit_message_text(
        f"Usuario {target.full_name}: {target.status}."
    )
    if status == "ACTIVE":
        try:
            await context.bot.send_message(
                chat_id=target.telegram_user_id,
                text="✅ Tu acceso al sistema ha sido autorizado.",
            )
        except Exception:
            pass
