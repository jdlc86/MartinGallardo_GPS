  ...
  if(req.mode==='navigate'&&url.pathname.includes('/preview-modern/')){
    event.respondWith((async()=>{
      ...
      try{
        ...
        if(res&&res.ok){const copy=res.clone();cache.put(cleanUrl(req),copy).catch(()=>{});}
        return res;
      }catch(e){
        return (await cache.match(cleanUrl(req)))||(await cache.match('./index.html'))||(await cache.match('./offline.html'));
      }
    })());
    return;
  }
