(function(){
  ...
  function isTabletDevice(){try{return matchMedia('(pointer: coarse)').matches&&Math.max(screen.width||0,screen.height||0)>=800&&Math.min(screen.width||0,screen.height||0)>=600}catch{return false}}
  ...
  function syncCameraFullscreenToOrientation(){const camera=document.querySelector('.cameraBox.on');if(!camera)return;try{const tg=window.Telegram?.WebApp;if(!tg)return;if(isTabletDevice()){if(!tg.isFullscreen)tg.requestFullscreen?.();return}const landscape=matchMedia('(orientation: landscape)').matches&&innerHeight<=600;if(landscape){if(!tg.isFullscreen)tg.requestFullscreen?.()}else if(tg.isFullscreen!==false)tg.exitFullscreen?.()}catch{}}
  ...
  window.addEventListener('orientationchange',queueCameraFullscreenSync);
  window.addEventListener('resize',queueCameraFullscreenSync);
