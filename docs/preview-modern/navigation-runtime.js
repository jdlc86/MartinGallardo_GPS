...
const routes={
  ...
  "resource-observability.html":{back:"./",backLabel:"Recursos del sistema",home:"./"},
  "shared-expediente.html":{back:"./",backLabel:"Expediente compartido",home:"./"},
  "factory-reset.html":{back:"./",backLabel:"Restablecimiento",home:"./"},
...
window.PMGNavigation={
  go(href,{replace=false}={}){replace?location.replace(href):location.assign(href)},
...
function initVisual(){
  ...
  shell.innerHTML='<div class="pmg-nav-bar"><button class="pmg-nav-btn" id="pmg-nav-back" type="button" aria-label="Atrás">'+backIcon+'</button><div class="pmg-nav-title">'+(cfg.backLabel||document.title||"")+'</div><button class="pmg-nav-btn" id="pmg-nav-home" type="button" aria-label="Inicio">'+homeIcon+'</button></div><div class="pmg-nav-tools" id="pmg-nav-tools"></div>';
  main.insertBefore(shell,main.firstChild);window.dispatchEvent(new CustomEvent('pmg:navigation-ready',{detail:{path}}));
